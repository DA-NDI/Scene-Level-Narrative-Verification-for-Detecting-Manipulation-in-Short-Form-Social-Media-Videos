"""
Run narrative manipulation pipeline on FakeTT:
- 50 real + 50 fake videos (balanced sample)
- Pipeline: scene detection -> OCR keyframes -> Gemini reasoning
- Output: fakett_pipeline_results.json with per-video predictions and summary metrics
"""
...
RESULTS_FILE = Path("fakett_pipeline_results.json")

N_REAL = 50
N_FAKE = 50
RANDOM_SEED = 42
MAX_SCENES_FOR_OCR = 6
...
def extract_ocr_from_frame(video_path: Path, timestamp_sec: float):
    try:
...
def build_prompt(entry, scenes, ocr_texts):
    ...
    scene_summary = ""
    for i, ((start_sec, end_sec), ocr_text) in enumerate(zip(scenes[:MAX_SCENES_FOR_OCR], ocr_texts[:MAX_SCENES_FOR_OCR])):
...
Assess whether the TikTok post is narratively manipulated (cheapfake-style recontextualization, misleading caption framing, false claim overlay, fabricated context) versus a normal authentic post.
...
def main():
    ...
    for idx, row in enumerate(sample, start=1):
        ...
        print(f"[{idx:>3}/{len(sample)}] {video_id} | gold={gold}", end=" | ")

        scenes = run_scene_detection(video_path)
        print(f"scenes={len(scenes)}", end=" | ")

        ocr_texts = []
        for (start_sec, end_sec) in scenes[:MAX_SCENES_FOR_OCR]:
            ocr_texts.append(extract_ocr_from_frame(video_path, (start_sec + end_sec) / 2))

        prompt = build_prompt(row, scenes, ocr_texts)
        pred_score, pred_label, reasoning = call_gemini(prompt)

        if pred_label is None:
            pred_label = "fake" if (pred_score is not None and pred_score >= 50) else "real"

        print(f"pred={pred_label} score={pred_score}")
